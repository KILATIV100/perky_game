...
function update() {
    ...
    updateParticles();
    checkCollisions();
    updateGameUI(); // Оновлюємо UI в кожному кадрі
...
function checkCollisions() {
    ...
    coffees = coffees.filter(coffee => {
        if (Math.hypot(player.x - coffee.x, player.y - coffee.y) < player.width) {
            currentCoffeeCount++;
            // updateGameUI(); // UI тепер оновлюється в update()
...
function updateGameUI() {
    const newHeight = Math.max(0, Math.floor((-player.y + canvas.height - 100) / 10)); // Конвертація в метри
...
async function initializeApp() {
    ...
    gameLoop(); // Запускаємо ігровий цикл один раз
}
